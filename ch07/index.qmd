...
How can we quantify a claim like "the true parameter is low"? One possibility is to compute the *mean* or *expected value* of the parameter, which is mathematically given by $\int x p(x) dx$ for a posterior distribution $p(x)$.
...
Credible intervals are related to, but shouldn't be mistaken for, the confidence intervals that are often used in frequentist statistics.^[Moreover, these confidence intervals themselves should definitely not be confused with $p$-values!]
...
We consider a model that fixes one of its parameters to a pre-specified value of interest (here $\mathcal{H}_0: p = 0.5$).
This is sometimes referred to as a *null hypothesis*.
The other model says that the parameter is free to vary.
In the classical hypothesis testing framework, we would write: $\mathcal{H}_1: p \neq 0.5$.
...
This idea is called the principle of parsimony or Occam's razor, and will be discussed at length [later in this book](../ch13/index.qmd).
...
This (perhaps surprising!) result was described by @dickey_weighted_1970, who attributed it to Leonard "Jimmie" Savage. The method is called the *Savage-Dickey density ratio* and is widely used in experimental science.
...
var savageDickeyDenomenator = expectation(
  complexModelPrior,
  function(x) {
...
Of course when we, as scientists, try to test our cognitive models of people, we can do so using BDA! This leads to more complex models in which we have an "outer" Bayesian data analysis model and an "inner" Bayesian cognitive model. (This is exactly the "nested Infer" pattern introduced for [Social cognition](../ch06/index.qmd)):
...
<!-- For an extended example of using these tools in WebPPL, see the Example: BDA for Tug of War. -->
