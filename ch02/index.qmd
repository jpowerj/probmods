...
### 2.3.2 Constructing Marginal Distributions: `Infer`
...
In general, the joint probability of two random choices $A$ and $B$ made sequentially, in that order, can be written as $\Pr(A,B) = \Pr(A) \Pr(B \vert A)$.
...
We can write this using probability notation as: $\Pr(A) = \sum_{B} \Pr(A,B)$, where we view $A$ as the final value and $B$ as a random choice on the way to that value.
...
## 2.8 Reading and Discussion

### 2.8.1 The Nature and Scope of Computational Models

Read Sec. 1.2 of Marr's classic book *Vision* [@marr_vision_1982]. Marr points out that when we ask a question about the mind (e.g., "How does language work?"), there are at least three different interrelated levels we can answer that question. Marr's three levels of analysis have become one of the key insights in the meta-science of the mind (that is, the science of conducting cognitive science), and we will be returning to them throughout the course. 

**Reading Questions:**

1. Many neuroscientists argue that learning involves changing the structure of the synapses of neurons. [Randy Gallistel](http://psych.rutgers.edu/faculty-profiles-a-contacts/96-charles-randy-gallistel) has argued that other mechanisms may be involved as well. At which of Marr's levels is this discussion happening?

2. Psychologists and artificial intelligence researchers both study the nature of intelligence. At which of Marr's levels are their interests most likely to interact?
